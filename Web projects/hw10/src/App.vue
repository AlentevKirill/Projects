...
export default {
    ...
    beforeCreate() {
      ...
      this.$root.$on("onRegister", (login, userName) => {
        if (userName.trim().length < 1 || userName.trim().length > 32) {
          this.$root.$emit("onRegisterValidationError", "Name size should be from 1 to 32 characters");
          return;
        }

        if (login.trim().length < 3 || login.trim().length > 16) {
          this.$root.$emit("onRegisterValidationError", "The login size should be from 3 to 16 characters");
          return;
        }


        const regexp = new RegExp('^[a-z]+$');
        if (!regexp.test(login.trim())) {
        ...
        const users = Object.values(this.users).filter(u => u.login === login);
        if (users.length !== 0) {
          this.$root.$emit("onRegisterValidationError", "This login already use");
        } else {
          const id = Math.max(...Object.keys(this.users)) + 1;
          this.$root.$set(this.users, id, {
            id: id, login: login, name: userName, admin: false
          });
