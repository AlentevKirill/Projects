...
public class UserRepositoryImpl implements UserRepository {
    ...
    public User findByLoginOrEmail(String loginOrEmail, String type) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM User WHERE "+ type +"=?")) {
    ...
    @Override
    public User findByLoginOrEmailAndPasswordSha(String loginOrEmail, String type, String passwordSha) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM User WHERE " + type + "=? AND passwordSha=?")) {
                statement.setString(1, loginOrEmail);
